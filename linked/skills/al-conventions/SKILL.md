...
- Satisfy CodeCop AA0248: qualify current-object global variables, labels, and procedures with `this.` wherever they are read or written from AL object code. Do not use `this.` for local variables, parameters, record fields, or standard globals such as `Rec`, `CurrPage`, and `CurrReport`.
